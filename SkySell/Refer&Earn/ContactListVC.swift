...
class ContactListVC: UIViewController, MFMailComposeViewControllerDelegate, MFMessageComposeViewControllerDelegate, UINavigationControllerDelegate {
    ...
    func SOGetPermissionContactAccess() {
        ...
        case .authorized:
            ...
            SOGetAddressBookRecord(type: contactType)
            break
        case .denied:
        ...
        case notDetermined:
            if let theBook: ABAddressBook = addressBook{
                ABAddressBookRequestAccessWithCompletion(theBook,
                                                         {granted, error in
                                                            if granted{
                                                                self.SOGetAddressBookRecord(type: self.contactType)
                                                            } else {
    ...
    func SOGetAddressBookRecord(type: FetchType) {
        ...
        let sortedResults: NSArray = arrContactList.sortedArrayUsingDescriptors([descriptor]) as NSArray
        self.initilizeTable(listArray: sortedResults as [AnyObject])
    }
    ...
    func initilizeTable(listArray: [AnyObject]) {
        ...
        for dict in listArray {
            ...
            let index: String.Index = strData.startIndex.advanced(1)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if contactType == .PhoneNumber {
            ...
            if arrPhone.count > 0 {
                let strPhone = arrPhone[0] as! NSString
                if arrSelectedContacts.contains(strPhone) {
                    arrSelectedContacts.remove(strPhone)
                }
                else {
                    arrSelectedContacts.add(strPhone)
                }
            }
        } else {
            let arrEmail = dict["email"] as! NSArray
            if arrEmail.count > 0 {
                ...
                if arrSelectedContacts.contains(strEmail) {
                    arrSelectedContacts.remove(strEmail)
                }
                else {
                    arrSelectedContacts.add(strEmail)
                }
            }
        }
    ...
    @IBAction func actionSend(sender: AnyObject) {
        if contactType == .PhoneNumber {
            presentModalMessageComposeViewController(animated: true)
        } else {
            presentModalMailComposeViewController(animated: true)
        }
    }
